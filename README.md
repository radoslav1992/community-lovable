...
docs/              seed-diskusii.md — чернови за първите теми в емисията
...
- **Проекти** — публична галерия (`/proekti`) с приложения, създадени с
  Lovable: всеки влязъл член добавя линк към живото си демо (адресът се
  нормализира и не се допускат дубликати), избира категория, до 4 етикета и
  по избор линк за ремикс (само `lovable.dev`). Избраният проект (`featured`)
  застава като голяма карта в началото на галерията и в сайдбара на началната
  страница; общността харесва проектите (работи и без JavaScript), а филтрите
  подреждат по категория или по най-харесвани. Проектите на един член се
  показват и в профила му. Авторът — и модератор/админ — може да премахне
  проект
